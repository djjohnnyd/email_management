"""
delete_old_emails.py  (OAuth2 / device code flow version)
----------------------------------------------------------
Connects via IMAP using OAuth2 and deletes emails that meet ALL criteria:
...
# Add or remove folders as needed. Run debug_imap_auth.py to see all folder names.
FOLDERS_TO_SCAN = [
    "INBOX",
    "Sent Items",
    "Archive",
...
def process_folder(mail, folder, cutoff_date_str, dry_run, limit):
    ...
    print(f"  Found {len(all_ids)} emails older than {YEARS_OLD} years")
...
def main():
    ...
    missing = []
    if "yourdomain.com" in EMAIL_ADDRESS:
        missing.append("EMAIL")
    if "your-client-id" in CLIENT_ID:
        missing.append("CLIENT_ID")
    if "your-tenant-id" in TENANT_ID:
        missing.append("TENANT_ID")
    if missing:
        print(f"ERROR: Please set the following in your .env file: {', '.join(missing)}")
        sys.exit(1)
    ...
    # Build auth bytes once — same pattern that works in debug script
